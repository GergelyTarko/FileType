...
func GetFromBuffer(buf []byte) (FileType, error) {
	...
	if hasPrefix(buf, 0x21, 0x3C, 0x61, 0x72, 0x63, 0x68, 0x3E, 0x0A) { // DEB
		if hasPrefix(buf, 0x21, 0x3C, 0x61, 0x72, 0x63, 0x68, 0x3E, 0x0A, 0x2F, 0x20, 0x20, 0x20) { // LIB
			// TODO Test for .a (Linux static Library)
			// I'm not even sure about the .lib signature
			return newType("lib", "application/octet-stream", "Windows Static Library"), nil
		}
	...
	if hasPrefix(buf, 0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0x1A, 0xE1) { //CFB
		...
		if hasPrefixOffset(buf, 512, 0x09, 0x08, 0x10, 0x00, 0x00, 0x06, 0x05, 0x00) { // XLS
			return newType("xls", "application/vnd.ms-excel", "MS Office Word excel file"), nil
		}
		if hasPrefixOffset(buf, 512, 0x52, 0x00, 0x6F, 0x00, 0x6F, 0x00, 0x74, 0x00, 0x20, 0x00, 0x45, 0x00, 0x6E, 0x00, 0x74, 0x00, 0x72, 0x00, 0x79, 0x00) { //MSG
			return newType("msg", "application/vnd.ms-outlook", "Microsoft Outlook and MS Exchange email message file"), nil
		}
